...
- **青龙面板集成**：可选从青龙面板环境变量自动拉取网易云、B 站、抖音 Cookie（详见 [iOS_Shortcut_Guide.md](./iOS_Shortcut_Guide.md)）。
...
4. **配置**
   ...
   访问 Web 界面 `http://localhost:3000` 的设置页，或直接修改 `config.js` / 环境变量：
...
| 接口 | 方法 | 说明 |
|------|------|------|
| `/api/process` | `POST` | 提交任务，Body: `{ "url": "...", "cookies": {...}, "qinglongConfig": {...} }`，返回 `{ "status": "processing", "taskId": "..." }` |
